...
## Install
rpFBA requires [RDKit](https://www.RDKit.org) which is not available through pip. It can be installed through Conda:
```sh
[sudo] conda install -c rdkit rdkit
```
### From pip
```sh
[sudo] python -m pip install rpfba
```
### From Conda
```sh
[sudo] conda install -c brsynth rpfba
```
...
**From Python code**
```python
from rpfba import rpFBA, build_args_parser

parser = build_args_parser()
args  = parser.parse_args()

rpfba = rpFBA(db=args.store_mode)
rpfba.rp2ToSBML(args.rp2_pathways,
